      ...
      results.forEach((result) => {
        if (result.i && (result.qid === "movie" || result.qid === "tvSeries")) {
          ...
          if (result.qid === "movie" && result.i) {
            imageAndInfo = `<a onClick="setUrl(this); return setVideo(this);" url="imdb=${result.id}&type=movie&title=${result.l.replace(/ /g, "_")}" isWebSeries="false" title="${result.l}"  class="links" IMDB="${result.id}" href="https://player.autoembed.cc/directstream.php?video_id=${result.id}" target="_blank">
            ...
            imageAndInfo = `<a onClick="setUrl(this); return setVideo(this);" url="imdb=${result.id}&season=1&episode=1&title=${result.l.replace(/ /g, "_")}" IMDB="${result.id}" title="${result.l}" isWebSeries="true" class="links" href="https://player.autoembed.cc/directstream.php?video_id=${result.id}&s=1&e=1" target="_blank">
                      <img alt="${result.l}" src="${optimisedImageUrl(result.i.imageUrl)}">
                        <div class="info">
                          <h3>${result.l}</h3>
                          <p>${result.s}</p>
...
function setAll(imdb, title, season, episode, type) {
  if (imdb && title && !season && !episode && type) {
    ...
    a.setAttribute("href", " https://player.autoembed.cc/directstream.php?video_id=" + imdb);
    ...
    a.setAttribute(
      "href",
      `https://player.autoembed.cc/directstream.php?video_id=${imdb}&s=${season}&e=${episode}`
    );
    a.setAttribute("target", "_blank");
    a.click();
  }
